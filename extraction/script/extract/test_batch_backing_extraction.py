"""
Batch extraction test for Backing type files
Configure TARGET_FILES to specify which files to extract

使用方法:
    python tests/test_batch_backing_extraction.py                    # 提取所有 PDF 文件
    python tests/test_batch_backing_extraction.py file1.pdf          # 提取指定文件
    python tests/test_batch_backing_extraction.py file1.pdf file2.pdf # 提取多个文件

功能:
    - 扫描 tests/files/ 目录中的所有 PDF 文件
    - 对每个 PDF 执行 BACKING 类型的提取
...
#   TARGET_FILES = ["backing_material.pdf"]  # Extract only specific file
#   TARGET_FILES = ["file1.pdf", "file2.pdf"]  # Extract multiple files
# ============================================================================
TARGET_FILES = None  # Extract all PDF files


async def extract_all_backing_files():
    """Extract all Backing type files from tests/files directory"""

    start_time = time.time()
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 Starting Batch Backing Extraction")
    ...
    try:
        ...
        if TARGET_FILES is None:
            ...
            logger.info(f"✅ Found {len(all_pdf_files)} PDF files to extract as Backing type\n")
        else:
        ...
        for idx, pdf_file in enumerate(all_pdf_files, 1):
            ...
            logger.info(f"   Document Type: BACKING")
            ...
            try:
                ...
                if result:
                    ...
                    # Print detailed AI output for physical_and_chemical_data
                    if "physical_and_chemical_data" in result:
                        logger.info(f"\n   📋 PHYSICAL AND CHEMICAL DATA DETAILS:")
                        data = result["physical_and_chemical_data"]

                        # Handle nested case
                        if isinstance(data, dict) and "physical_and_chemical_data" in data:
                            data = data["physical_and_chemical_data"]

                        if isinstance(data, list):
                            logger.info(f"   Total properties: {len(data)}\n")
                            for idx, item in enumerate(data, 1):
                                logger.info(f"   [{idx}] Property: {item.get('property', 'N/A')}")
                                logger.info(f"       Tesa: value={item.get('tesa_test_figures_value')}, tolerance={item.get('tesa_test_figures_tolerance')}, unit={item.get('tesa_test_figures_unit')}, standard={item.get('tesa_standard')}")
                                logger.info(f"       Supplier: value={item.get('supplier_test_figures_value')}, tolerance={item.get('supplier_test_figures_tolerance')}, unit={item.get('supplier_test_figures_unit')}, standard={item.get('supplier_standard')}")
                        else:
        ...
        summary_file = output_dir / "batch_backing_extraction_summary.json"
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump({
                "document_type": "backing",
                "total": len(results),
...
if __name__ == "__main__":
    asyncio.run(extract_all_backing_files())
