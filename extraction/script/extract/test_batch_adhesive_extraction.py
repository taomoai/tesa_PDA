"""
Batch extraction test for Adhesive type files
Configure TARGET_FILES to specify which files to extract

使用方法:
    python tests/test_batch_adhesive_extraction.py                    # 提取所有 PDF 文件
    python tests/test_batch_adhesive_extraction.py file1.pdf          # 提取指定文件
    python tests/test_batch_adhesive_extraction.py file1.pdf file2.pdf # 提取多个文件

功能:
    - 扫描 tests/files/ 目录中的所有 PDF 文件
    - 对每个 PDF 执行 ADHESIVE 类型的提取
...
#   TARGET_FILES = ["adhesive_material.pdf"]  # Extract only specific file
#   TARGET_FILES = ["file1.pdf", "file2.pdf"]  # Extract multiple files
# ============================================================================
TARGET_FILES = None  # Extract all PDF files


async def extract_all_adhesive_files():
    """Extract all Adhesive type files from tests/files directory"""

    start_time = time.time()
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 Starting Batch Adhesive Extraction")
    ...
    try:
        ...
        if TARGET_FILES is None:
            ...
            logger.info(f"✅ Found {len(all_pdf_files)} PDF files to extract as Adhesive type\n")
        else:
        ...
        for idx, pdf_file in enumerate(all_pdf_files, 1):
            ...
            logger.info(f"   Document Type: ADHESIVE")
            ...
            try:
                ...
                result = await service.extract_pdf_to_json(
                    ...
                    doc_type=DocumentType.ADHESIVE
                )
                ...
                if result:
                    ...
                    if "product_info" in result:
                        ...
                        logger.info(f"   Company: {product_info.get('company', 'N/A')}")
                        logger.info(f"   NART: {product_info.get('nart', 'N/A')}")
                        logger.info(f"   Product ID: {product_info.get('id', 'N/A')}")
                        logger.info(f"   Material Type: {product_info.get('material_type', 'N/A')}")
                        logger.info(f"   Version: {product_info.get('version', 'N/A')}")
                        logger.info(f"   Status: {product_info.get('status', 'N/A')}")

                    # Print product components
                    if "product_components" in result:
                        logger.info(f"\n   📋 PRODUCT COMPONENTS DETAILS:")
                        components_data = result["product_components"]
                        
                        # Handle nested case
                        if isinstance(components_data, dict) and "components" in components_data:
                            components = components_data["components"]
                        else:
                            components = components_data.get("components", []) if isinstance(components_data, dict) else []

                        if isinstance(components, list):
                            logger.info(f"   Total components: {len(components)}\n")
                            for idx, component in enumerate(components, 1):
                                logger.info(f"   [{idx}] NART: {component.get('nart', 'N/A')}")
                                logger.info(f"       Product ID: {component.get('product_identification', 'N/A')}")
                                logger.info(f"       Solids Content: {component.get('solids_content_kg', 'N/A')} kg")
                                logger.info(f"       Weight: {component.get('weight_of_contents_kg', 'N/A')} kg")
                                logger.info(f"       Tolerance: {component.get('tolerance_percent', 'N/A')}%")
                        else:
                            logger.info(f"   Raw data: {json.dumps(components_data, indent=6, ensure_ascii=False)}")

                    # Print characteristics and properties
                    if "characteristics_and_properties" in result:
                        logger.info(f"\n   📋 CHARACTERISTICS AND PROPERTIES DETAILS:")
                        char_data = result["characteristics_and_properties"]
                        
                        # Handle nested case
                        if isinstance(char_data, dict) and "characteristics" in char_data:
                            characteristics = char_data["characteristics"]
                        else:
                            characteristics = char_data.get("characteristics", []) if isinstance(char_data, dict) else []

                        if isinstance(characteristics, list):
                            logger.info(f"   Total characteristics: {len(characteristics)}\n")
                            for idx, char in enumerate(characteristics, 1):
                                logger.info(f"   [{idx}] Item No: {char.get('item_no', 'N/A')}")
                                logger.info(f"       Description: {char.get('description', 'N/A')}")
                                logger.info(f"       Unit: {char.get('unit', 'N/A')}")
                                logger.info(f"       Target Value: {char.get('target_value', 'N/A')} {char.get('target_value_unit', '')}")
                                logger.info(f"       Test Method: {char.get('test_method', 'N/A')}")
                                logger.info(f"       Test Type: {char.get('test_type', 'N/A')}")
                        else:
                            logger.info(f"   Raw data: {json.dumps(char_data, indent=6, ensure_ascii=False)}")
        ...
        summary_file = output_dir / "batch_adhesive_extraction_summary.json"
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump({
                "document_type": "adhesive",
                "total": len(results),
...
if __name__ == "__main__":
    asyncio.run(extract_all_adhesive_files())
